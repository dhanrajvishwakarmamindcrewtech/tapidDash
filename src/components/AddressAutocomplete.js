...
const GOOGLE_MAPS_API_KEY = process.env.REACT_APP_GOOGLE_MAPS_API_KEY || 'AIzaSyBMrMxMvd1ZpzRJo07IqTj-FqWq3qbnKK4';
const DEBUG = process.env.NODE_ENV !== 'production';

const AddressAutocomplete = ({ value, onChange, onSelect, inputClassName = '', placeholder = 'Search address' }) => {
  ...
  const getPlaceDetails = async (placeId, description) => {
    try {
      if (DEBUG) console.log('🔍 Fetching COMPREHENSIVE place details using Places API (New) for:', placeId);
      ...
      const response = await fetch(`https://places.googleapis.com/v1/places/${placeId}`, {
        ...
        headers: {
          ...
          'X-Goog-FieldMask': [
            // === BASIC INFORMATION ===
            'id', 'name', 'displayName', 'formattedAddress', 'shortFormattedAddress',
            
            // === LOCATION & GEOMETRY ===
            'location', 'viewport', 'addressComponents', 'plusCode', 'googleMapsUri',
            ...
            // === HOURS & OPERATIONS ===
            'regularOpeningHours', 'currentOpeningHours', 'regularSecondaryOpeningHours',
            ...
            // === SERVICE OPTIONS ===
            'curbsidePickup', 'delivery', 'dineIn', 'takeout', 'reservable',
            ...
            'servesVegetarianFood', 'servesWine',
            
            // === ADDITIONAL FEATURES ===
            'allowsDogs', 'goodForChildren', 'goodForGroups', 'goodForWatchingSports',
            'liveMusic', 'menuForChildren', 'outdoorSeating', 'restroom',
            'paymentOptions', 'parkingOptions',
            
            // === VISUAL & MEDIA ===
            'photos', 'iconMaskBaseUri', 'iconBackgroundColor',
            
            // === AI-POWERED SUMMARIES & FEATURES ===
            'editorialSummary', 'generativeSummary', 'reviewSummary',
            
            // === ADDITIONAL DETAILS ===
            'adrFormatAddress', 'attributions', 'containingPlaces', 'subDestinations'
          ].join(',')
        }
      });
      ...
      if (DEBUG) console.log('🏢 RAW Places API (New) Response - Full Data:', place);
      ...
      const placeData = {
        ...
        regular_opening_hours: place.regularOpeningHours ? {
          ...
          secondary_hours_type: place.regularOpeningHours.secondaryHoursType,
        ...
        photos: place.photos?.map((photo, index) => ({
          ...
          author_attributions: photo.authorAttributions?.map(attr => ({
      ...
      if (DEBUG) console.log('🎉 PLACES API (NEW) - MAXIMUM COMPREHENSIVE DATA:', placeData);
      if (DEBUG) console.log('⭐ Enhanced Review Intelligence:', {
        ...
        review_languages: [...new Set(placeData.reviews?.map(r => r.text_language).filter(Boolean))],
        has_translated_reviews: placeData.reviews?.some(r => r.translated),
        author_photo_count: placeData.reviews?.filter(r => r.author_attribution?.photo_uri).length || 0
      });
      if (DEBUG) console.log('📸 Enhanced Photo Intelligence:', {
        total_photos: placeData.photos?.length || 0,
        photo_resolutions: placeData.photos?.length > 0 ? ['400px (small)', '800px (medium)', '1600px (large)', 'original resolution'] : [],
        has_author_attributions: placeData.photos?.some(p => p.author_attributions?.length > 0),
        max_resolution: placeData.photos?.length > 0 ? `${Math.max(...placeData.photos.map(p => p.width_px))}x${Math.max(...placeData.photos.map(p => p.height_px))}` : 'N/A',
        photographer_count: [...new Set(placeData.photos?.flatMap(p => p.author_attributions?.map(a => a.display_name)).filter(Boolean))].length
      });
      if (DEBUG) console.log('🍽️ Complete Restaurant Intelligence:', {
        meal_services: {
      ...
      if (DEBUG) console.log('🏢 Advanced Business Intelligence:', {
      ...
      if (DEBUG) console.log('🤖 AI-Powered Intelligence & Context:', {
      ...
      placesService.current.getDetails(
        {
          ...
          fields: [
            ...
            'types', 'rating', 'user_ratings_total', 'reviews', 'photos',
